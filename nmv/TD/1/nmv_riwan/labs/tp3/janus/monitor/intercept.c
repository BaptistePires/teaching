...
__attribute__((
	aligned(0x1000))) static char guest_exit_stack[HANDLER_STACK_SIZE];
...
static uint64_t read_register(xed_reg_enum_t reg, ucontext_t *uc)
{
	...
	switch (enclosing) {
	case XED_REG_RAX:
		val = uc->uc_mcontext.gregs[REG_RAX];
		break;
	case XED_REG_RBX:
	...
	case XED_REG_RSI:
		...
		break;
	case XED_REG_RBP:
		val = uc->uc_mcontext.gregs[REG_RBP];
		break;
	case XED_REG_RSP:
...
static void write_register(xed_reg_enum_t reg, ucontext_t *uc, uint64_t val)
{
	...
	switch (enclosing) {
	case XED_REG_RAX:
		tmp = &uc->uc_mcontext.gregs[REG_RAX];
		break;
	case XED_REG_RBX:
		tmp = &uc->uc_mcontext.gregs[REG_RBX];
		break;
	case XED_REG_RCX:
		tmp = &uc->uc_mcontext.gregs[REG_RCX];
		break;
	case XED_REG_RDX:
		tmp = &uc->uc_mcontext.gregs[REG_RDX];
		break;
	case XED_REG_RDI:
		tmp = &uc->uc_mcontext.gregs[REG_RDI];
		break;
	case XED_REG_RSI:
		tmp = &uc->uc_mcontext.gregs[REG_RSI];
		break;
	case XED_REG_RBP:
		tmp = &uc->uc_mcontext.gregs[REG_RBP];
		break;
	case XED_REG_RSP:
		tmp = &uc->uc_mcontext.gregs[REG_RSP];
		break;
	case XED_REG_R8:
		tmp = &uc->uc_mcontext.gregs[REG_R8];
		break;
	case XED_REG_R9:
		tmp = &uc->uc_mcontext.gregs[REG_R9];
		break;
	case XED_REG_R10:
		tmp = &uc->uc_mcontext.gregs[REG_R10];
		break;
	case XED_REG_R11:
		tmp = &uc->uc_mcontext.gregs[REG_R11];
		break;
	case XED_REG_R12:
		tmp = &uc->uc_mcontext.gregs[REG_R12];
		break;
	case XED_REG_R13:
		tmp = &uc->uc_mcontext.gregs[REG_R13];
		break;
	case XED_REG_R14:
		tmp = &uc->uc_mcontext.gregs[REG_R14];
		break;
	case XED_REG_R15:
		tmp = &uc->uc_mcontext.gregs[REG_R15];
		break;
	default:
		emulation_failure(uc);
	}
...
static void emulate_out(xed_decoded_inst_t *inst, ucontext_t *uc)
{
	...
	case 8:
		...
		break;
	case 16:
		out16(port, (uint16_t)val);
		break;
	case 32:
		out32(port, (uint32_t)val);
		break;
	default:
		emulation_failure(uc);
	}
...
static void emulate(siginfo_t *si, ucontext_t *uc)
{
	...
	case XED_ICLASS_HLT:
		display_vga();
		display_page_table_paddr();
		display_shadow_tables();
		display_mapping();
		exit(EXIT_SUCCESS);
		break;
	default:
...
static void handler(int signum __attribute__((unused)), siginfo_t *si,
		    ucontext_t *uc)
{
	uint8_t vec;
	uint64_t code;
