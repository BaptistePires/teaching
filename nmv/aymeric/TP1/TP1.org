#+TITLE: TP1 NMV : Gestion de la mémoire virtuelle
...
(vaddr & (0x1FF << 12 + (lvl - 1) * 9)) >> 12 + (lvl - 1) * 9
...
La totalité des pages d'adresse entre ~0x0~ et ~0x40000000~ sont toutes
...
virtuelle ~0x2000000000~) et le bss juste au-dessus, et pour l'allocation
...
des sections est l'adresse virtuelle ~0x2000000000~. Donc on peut et *on doit*
placer ces sections au-dessus de ~0x2000000000~.

La taille du payload est donnée par la différence entre ~ctx->load_end_paddr~ et
~ctx->load_paddr~ (on admet la contiguité).

Donc le payload ira de l'adresse virtuelle ~0x2000000000~ à l'adresse virtuelle
~0x2000000000~ + (~ctx->load_end_paddr~ - ~ctx->load_paddr~), et bss ira de
l'adresse virtuelle ~0x2000000000~ + (~ctx->load_end_paddr~ - ~ctx->load_paddr~)
à l'adresse virtuelle ~ctx->bss_end_vaddr~.
...
- Allouer une page pour PML4, et mettre l'adresse de cette page dans le CR3 de
  la nouvelle tâche (donc dans ~ctx->pgt~).
...
- Mapper grâce à la fonction ~map_page()~ les pages du payload. Les adresses
  virtuelles commenceront à ~ctx->load_vaddr~.
...
#+BEGIN_SRC c
  void load_task(struct task *ctx)
  {
	  ...
	  /* Allocation nouveau PML3 */
	  p = (uint64_t *)ctx->pgt;
	  paddr = alloc_page();
	  memset((void *)paddr, 0, 4096);
	  p[0] |= (paddr | PGT_VALID_MASK | PGT_WRITABLE_MASK | PGT_USER_MASK);

	  /* Recopie ancien PML3[0] vers nouveau PML3[0] */
...
*** Aparté allocation paresseuse

On remarque ici qu'il n'y a pas d'allocation paresseuse dans Rackdoll. Si la
fonction ~mmap~ est celle qui est effectivement exécutée (plusieurs fois
potentiellement, puisqu'elle ne mappe qu'une seule page) quand l'appel système
~mmap(2)~ est appelé, il n'y a pas d'allocation paresseuse. On voit que
l'allocation et le mapping est fait de suite.

On voit donc comment l'allocation paresseuse peut être implémentée. Un appel à
~mmap(2)~ dans linux ne signifie pas la création d'un mapping dans la table des
pages. On imagine une liste intermédiaire de correspondances à la manière de ce
qui est visible dans ~/proc/<pid>/maps~.

La MMU, qui constate une absence de correspondance pour une adresse virtuelle
particulière, déclenche une interruption qui est traitée par le système (en
l'occurrence, la fonction ~pgfault()~).

L'équivalent de la fonction ~pgfault()~ de linux, pour implémenter, vérifierait
donc deux choses :
- Est-ce que l'adresse virtuelle est de la pile ?
- Est-ce que l'adresse virtuelle est quelque part dans les correspondances
  potentielles de ~/proc/<pid>/maps~ ?

Si la réponse à une seule de ces deux question est oui, alors ~pgfault()~ fait
bien l'allocation de la page correspondante.
